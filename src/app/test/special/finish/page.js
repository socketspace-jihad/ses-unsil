...
export default function Home() {
  ...
  useEffect(() => {
    axios.get("/api/special/test-tpa-result", {
      params: {
      ...
      .then(resp => {
        ...
        setExam(data);
        setCategorized(resp.data.categorized[0]);
        setNarration(`
            Selamat!, Kamu Sudah menyelesaikan Tes Potensi Akademik ${data.name}. 
            Skor Tes Potensi Akademik kamu adalah ${data.score}.
            Kami sudah menemukan kriteria tingkat kesulitan Tes Mata Kuliah untukmu. 
  ...
  const handleFinish = () => {
    ...
    axios.post("/api/special/create-mahasiswa-test-matkul",{
        "test_matkul_id": params.get("test_matkul_id"),
        "test_matkul_categorized_id": categorized.id,
        "kelas":kelas
    })
    .then(resp=>{
        router.push(`/test/special/matkul/detail?test_matkul_id=${params.get("test_matkul_id")}&mahasiswa_test_matkul_id=${resp.data.insertId}&kelas=${kelas}&test_matkul_categorized_id=${categorized.id}`);
    })
    .catch(err=>{
        console.log(err)
    })
